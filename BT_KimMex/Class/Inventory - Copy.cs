...
//            {
    ...
    //                      join proj in db.tb_project on site.site_id equals proj.site_id
    //                      join it in db.tb_item_request on proj.project_id equals it.ir_project_id
    //                      join po in db.tb_purchase_order on it.ir_id equals po.item_request_id
    ...
    //                                  //total_quantity = inv.total_quantity,
    //                                  //in_quantity = inv.in_quantity,
    //                                  out_quantity = transfer.quantity, // transfer.remain_quantity,
    ...
    //                                             join pr in db.tb_purchase_order on mr.ir_id equals pr.item_request_id
    //                                             where string.Compare(pr.purchase_order_id, itemReceive.ref_id) == 0
    ...
    //        using (kim_mexEntities db = new kim_mexEntities())
    //        {
    //            //var inventoryList = (from inv in db.tb_inventory
    //            //               join item in db.tb_product on inv.product_id equals item.product_id
    //            //               join wah in db.tb_warehouse on inv.warehouse_id equals wah.warehouse_id
    //            //               orderby item.product_code
    //            //               where string.Compare(inv.ref_id, itemReceive.receive_item_voucher_id) == 0
    //            //               select new
    //            //               {
    //            //                   inventory_id = inv.inventory_id,
    //            //                   inventory_date = inv.inventory_date,
    ...
    //                }
    //            }
    //            ItemRequest.UpdateCompletedItemRequest(id);
    //        }
    //    }
    ...
    //    }
        ...
        //                Select(x => new InventoryDetailViewModel() { inventory_item_id = x.inventory_item_id, quantity = x.quantity, unit = x.unit }).ToList();
        //        }
        //        else
   ...
   //                             join item in db.tb_product on inv.st_item_id equals item.product_id
   //                             orderby item.product_code
   //                             where string.Compare(inv.st_ref_id, id) == 0 && inv.status == true && inv.remain_quantity > 0
   //                             select new InventoryViewModel()
   //                             {
   //                                 inventory_id = inv.st_detail_id,
   //                                 //inventory_date = inv.inventory_date,
   ...
   //             var item = (from a in db.tb_inventory_detail join b in db.tb_stock_issue_return on a.inventory_ref_id equals b.stock_issue_return_id join c in db.tb_stock_transfer_voucher on b.stock_issue_ref equals c.stock_transfer_id join d in db.tb_stock_transfer_detail on c.stock_transfer_id equals d.st_ref_id where c.stock_transfer_id == id && d.status == true select new { a,b,c,d }).ToList();
   //             foreach (var issueItem in item)
   //                 if (issueItem.d.remain_quantity > 0) countNotCompleted++;
   ...
   //             if (string.IsNullOrEmpty(item_status))
   //             {
   //                 transferFromMainStockItems = db.tb_stock_transfer_detail.Where(x => string.Compare(x.st_ref_id, stId) == 0).Select(x => new Models.TransferFromMainStockItemViewModel()
   //                 {
   //                     st_item_id = x.st_item_id,
   //                     quantity = x.quantity,
   //                     itemUnit = x.unit,
   ...
   //             else
   //             {
   //                 transferFromMainStockItems = db.tb_stock_transfer_detail.Where(x => string.Compare(x.st_ref_id, stId) == 0 && string.Compare(x.item_status, item_status) == 0).Select(x => new Models.TransferFromMainStockItemViewModel()
   //                 {
   //                     st_item_id = x.st_item_id,
   //                     quantity = x.quantity,
   //                     itemUnit = x.unit,
   ...
   //             foreach (TransferFromMainStockItemViewModel itemTransfers in transferFromMainStockItems)
   //             {
   //                 string irDetail2Id = itemRequests.Where(x => string.Compare(x.ir_item_id, itemTransfers.st_item_id) == 0).Select(x => x.ir_detail2_id).FirstOrDefault();
   //                 if (irDetail2Id != null)
   //                 {
   //                     tb_ir_detail2 irDetail2 = db.tb_ir_detail2.Find(irDetail2Id);
   //                     if (isRollback)
   //                     {
   //                         irDetail2.remain_qty = irDetail2.remain_qty + Class.CommonClass.ConvertMultipleUnit(itemTransfers.st_item_id, itemTransfers.itemUnit, Convert.ToDecimal(itemTransfers.quantity), irDetail2.ir_item_unit);
   //                     }
   //                     else
   //                     {
   //                         irDetail2.remain_qty = irDetail2.remain_qty - Class.CommonClass.ConvertMultipleUnit(itemTransfers.st_item_id, itemTransfers.itemUnit, Convert.ToDecimal(itemTransfers.quantity), irDetail2.ir_item_unit);
